...
import { playSlotsConfig, numberOfWeeksToDisplayPlaySlots } from '@/config/appConfig';
...
const getNextOccurrences = (dayOfWeek: number, count: number): string[] => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (i === 0 && currentDate.getDay() === dayOfWeek && currentDate > new Date(new Date().setHours(0,0,0,0)) ) { 
      // Se hoje é o dia da semana e ainda não passou, considera hoje
      // Mas como queremos para "próximas semanas", vamos avançar para a próxima ocorrência se for hoje,
      // A menos que a lógica de "próxima semana" seja estrita.
      // Para simplicidade, vamos pegar a "próxima" ocorrência a partir de hoje.
       nextOccurrenceDate = nextDay(currentDate, dayOfWeek);
       currentDate = nextOccurrenceDate; // Atualiza para a próxima busca
    } else {
       nextOccurrenceDate = nextDay(currentDate, dayOfWeek);
...
export default function PlayPage() {
  ...
  const upcomingPlaySlots = playSlotsConfig.map(slot => ({
    ...
    dates: getNextOccurrences(slot.dayOfWeek, numberOfWeeksToDisplayPlaySlots)
            .map(dateStr => ({
                date: dateStr,
                // Adiciona a data formatada para exibição
                formattedDate: format(new Date(dateStr + 'T00:00:00'), "EEEE, dd 'de' MMMM", { locale: ptBR })
            }))
            // Filtra datas que já passaram (se getNextOccurrences puder retornar hoje e já passou o horário)
            // Como getNextOccurrences já pega "próximas", este filtro pode ser redundante, mas é seguro.
    ...
    <div className="w-full max-w-5xl mx-auto py-8 px-4 sm:px-6 lg:px-8 space-y-12">
      ...
      <section className="space-y-8">
        <h2 className="text-3xl font-semibold text-primary text-center mb-8 flex items-center justify-center gap-2">
          <CalendarDays /> Próximas Sessões "Play"
        </h2>
        ...
        {!authLoading && upcomingPlaySlots.map((slotType, typeIndex) => (
          slotType.dates.length > 0 && (
            <div key={slotType.key} className="space-y-6">
              {/* Não precisa de título para cada tipo de slot, já que o card mostra */}
              <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
                {slotType.dates.map(dateInfo => (
                  <PlaySlotDisplay
                    key={`${slotType.key}-${dateInfo.date}`}
                    slotConfig={slotType}
                    date={dateInfo.date} // Passa a data YYYY-MM-DD
      ...
      <Card className="shadow-lg bg-card">
        ...
        <CardContent className="space-y-6 text-foreground/80">
          ...
          <div>
            ...
            <p>
              As vagas são limitadas a {maxParticipantsPerPlaySlot} participantes por sessão para garantir uma ótima experiência de jogo. Faça sua inscrição individualmente através desta página para garantir sua vaga. É necessário estar logado.
            </p>
          </div>
          <div>
            <h3 className="font-semibold text-primary mb-1">Formato da Sessão</h3>
            <p>
              Durante as sessões "Play", os jogos são organizados de forma dinâmica, geralmente em formato de rodízio ou desafios, garantindo que todos os participantes joguem bastante e interajam com diferentes parceiros e oponentes. O foco é a diversão, a prática esportiva e a socialização!
            </p>
          </div>
          <div>
            <h3 className="font-semibold text-primary mb-1">Exclusividade dos Horários</h3>
            <p>
              Os horários dedicados às sessões "Play" (Sexta, Sábado e Domingo, das 16:00 às 20:00) são exclusivos para esta modalidade. Durante esses períodos, as quadras utilizadas para o "Play" não estarão disponíveis para aluguel avulso, garantindo o espaço para os participantes inscritos.
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      <section className="text-center mt-12 py-10 bg-gradient-to-r from-primary/5 via-background to-background rounded-lg shadow-sm border border-border/50">
        ...
        <Button size="lg" className="bg-accent hover:bg-accent/90 text-accent-foreground shadow-md hover:shadow-lg transition-shadow" asChild>
          <Link href="#"> {/* Idealmente, um link para o topo da seção de inscrição ou âncora */}
